...
class MessageQueue {
  ...
  retryHandler = (recurse = true) => {
    ...
    if (message.awaitingUpload) {
      console.log('skipping awaiting', message);
    ...
    this._sendMessage(meProfileId, conversationId, message)
      .then(() => {
        console.log('sending message succeeded', message);
  ...
  _sendMessage = (meProfileId, conversationId, _message) => {
    let message;
    if (_message.type === 0) {
      message = _message;
    }
    if (_message.type === 1) {
      message = {
        message: JSON.stringify(_message.image),
        type: _message.type,
      };
    }
    return sendMessage({
  ...
  updateQueuedMessage = (qid, updates) => {
    ...
    console.log('updated queue', this.resendQueue);
