...
import {
  CACHE as CACHE_MESSAGE,
  MULTI_CACHE,
} from '~/apis/messageCache/actions';
import { RECEIVE, REMOVE } from '~/apis/instantMessages/actions';
import { RECEIVE_MESSAGES, QUEUE_MESSAGE } from '~/apis/conversation/actions';
...
export function cacheMessages(messages) {
  store.dispatch(MULTI_CACHE(messages));
}
...
export function addQueuedMessage(conversationId, message) {
  ...
  cacheMessage(_message);
  store.dispatch(QUEUE_MESSAGE(conversationId, _message));
}
